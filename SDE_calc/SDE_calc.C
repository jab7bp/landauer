...
vector<double> dataDDE, dataLDE, dataBeta, dataSDE, element_sums;
...
void loadData(const char* filename, vector<double>& OW, vector<double>& PL, vector<double>& Al, vector<double>& Cu, vector<double>& SDE, vector<double>& DDE, int percentage = 100) {
    ...
    while (getline(file, line)) {
        ...
        double ow, pl, al, cu, dde, lde, sde, beta;
        ss >> ow >> pl >> al >> cu >> dde >> sde;
        // if( sde > 525 || sde < 500){
        //     continue;
        // }
        OW.push_back(ow);
...
vector<double> OW, PL, Al, Cu, SDE, DDE;
...
void analyze(const vector<double>& oldCoefficients, double percent_data_to_scan = 100) {
    ...
    // TString datafile = "All_data_0deg_Angled_Mixed_No_exception.csv";
    // TString datafile = "0deg_All_Photons_All_mixed_data.csv";
    // TString datafile = "0deg_All_Photons_only_data.csv";
    // TString datafile = "0deg_All_Mixed_only.csv";
    // TString datafile = "0deg_All_PL_Photons_only.csv";
    // TString datafile = "0deg_All_PM_Photons_only.csv";
    // TString datafile = "0deg_All_PH_Photons_data.csv";
    // TString datafile = "0deg_BL_only.csv";
    // TString datafile = "0deg_BH_only.csv";
    // TString datafile = "0deg_Mixed_L50_M30_NS30_only.csv";
    // TString datafile = "0deg_Mixed_NS20_only.csv";
    // TString datafile = "0deg_Mixed_EXCEPT_L50_M30_NS20_NS30.csv";
    // TString datafile = "0deg_Mixed_PL_only.csv";
    // TString datafile = "0deg_Mixed_PM_only.csv";
    // TString datafile = "0deg_Mixed_PH_only.csv";
    // TString datafile = "HighE_Angled_and_0deg_All_Mixed_data.csv";
    // TString datafile = "0deg_Mixed_L50_NS30_only.csv";
    // TString datafile = "0deg_Mixed_L50_NS30_M50_only.csv";
    // TString datafile = "0deg_Mixed_M30_only.csv";
    // TString datafile = "HighE_Angled_and_0deg_Mixed_EXCEPT_L50_M30_NS20_NS30.csv";
    // TString datafile = "HighE_Angled_and_0deg_Mixed_EXCEPT_L50_M30_M50_NS20_NS30.csv";
    // TString datafile = "0deg_All_Photons_All_mixed_HighE_Angular_data.csv";
    // TString datafile = "OTHER_data_types.csv";
    // TString datafile = "0deg_BL_BH_SDE_mixtures.csv";
    // TString datafile = "0deg_All_Photons_only_data_01_10_2024.csv";
    // TString datafile = "0deg_All_PL_Photons_only_01_10_2024.csv";
    // TString datafile = "0deg_Mixed_PH_only_01_10_2024.csv";
    // TString datafile = "0deg_PL_PM_all_betas_mixtures_01_10_2024.csv";
    // TString datafile = "0deg_All_Photons_and_All_Mixture_types_01_10_2024.csv";
    // TString datafile = "0deg_All_Photons_All_mixed_data.csv";
    // TString datafile = "0deg_BH_only_01_10_2024.csv";
    // TString datafile = "0deg_BL_only_01_10_2024.csv";
    TString datafile = "0deg_BL_only_04_10_2024.csv";

    const char *filename = datafile.Data();

    loadData(filename, OW, PL, Al, Cu, SDE, DDE, percent_data_to_scan);
    ...
    TVectorD y(n);     // Vector for SDE/DDE values
    ...
    cout << "-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*" << endl;
    cout << "-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*" << endl;
    cout << "SDE SDE SDE SDE SDE SDE SDE SDE SDE SDE " << endl << endl;

    // Print out the refined coefficients
    cout << "SDE Refined coefficients: OW = " << beta[0] << ", PL = " << beta[1] << ", Al = " << beta[2]<< ", Cu = " << beta[3] << endl;

    cout << endl;

    for( int coeff = 0; coeff < 4; coeff++ ){
        cout << "c" << coeff +1 << " = " << Form("%.5f", beta[coeff]) << endl;
    }
    ...
    // Plot the comparison
    TCanvas *c1 = new TCanvas("c1", "SDE Comparison", 800, 600);
    grInitial = new TGraph(n);
    grDDEInitial = new TGraph(n);
    grRefined = new TGraph(n);
    grDDERefined = new TGraph(n);
    ...
    grRefined->SetTitle("Comparison of Nominal and Refined SDE Calculations;Data Point;Calibrated SDE Value");
    grRefined->GetYaxis()->SetTitleOffset(1.3);
    grRefined->Draw("AP");
    ...
    cout << endl;
    cout << "Working on DDE now....." << endl;
    cout << endl;

    // Perform linear regression to refine coefficients
    TMatrixD ddeX(n, 4);  // Matrix for measurements
    TVectorD ddey(n);     // Vector for SDE/DDE values
    ...
    TMatrixD ddeXt(TMatrixD::kTransposed, X);
    TMatrixD ddeXtX = ddeXt * ddeX;
    TMatrixD ddeXtX_inv = ddeXtX.Invert();
    TVectorD ddebeta = ddeXtX_inv * (ddeXt * ddey);

    cout << "-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*" << endl;
    cout << "-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*" << endl;
    cout << "DDE DDE DDE DDE DDE DDE DDE DDE DDE DDE " << endl << endl;

    // Print out the refined coefficients
    cout << "DDE Refined coefficients: OW = " << ddebeta[0] << ", PL = " << ddebeta[1] << ", Al = " << ddebeta[2]<< ", Cu = " << ddebeta[3] << endl;

    cout << endl;

    for( int coeff = 0; coeff < 4; coeff++ ){
        cout << "c" << coeff +1 << " = " << Form("%.5f", ddebeta[coeff]) << endl;
    }

    // Calculate refined predicted SDE values
    vector<double> refinedPredictedDDE(n);
    for (int i = 0; i < n; ++i) {
        refinedPredictedDDE[i] = (ddebeta[0] * OW[i] + ddebeta[1] * PL[i] + ddebeta[2] * Al[i]+ ddebeta[3] * Cu[i]);
    }

    // Calculate initial and refined mean squared errors
    double ddeinitialError = 0, dderefinedError = 0;
    for (int i = 0; i < n; ++i) {
        dderefinedError += TMath::Power(refinedPredictedDDE[i] - DDE[i], 2);
    }
    dderefinedError /= n;

    for (int i = 0; i < n; ++i) {
        grDDEInitial->SetPoint(i, i, dataDDE[i]);
        grDDERefined->SetPoint(i, i, refinedPredictedDDE[i]);
    }

    TCanvas *c2 = new TCanvas("c2", "DDE Comparison", 600, 500);

    grDDERefined->SetMarkerStyle(22);
    grDDERefined->SetMarkerColor(kBlue);
    grDDERefined->SetTitle("Comparison of Nominal and Refined DDE Calculations;Data Point;Calibrated DDE Value");
    grDDERefined->GetYaxis()->SetTitleOffset(1.3);
    grDDERefined->Draw("AP");

    grDDEInitial->SetMarkerColor(kRed);
    grDDEInitial->SetMarkerStyle(20);
    grDDEInitial->SetMarkerSize(0.5);
    grDDEInitial->Draw("P same");
...
void SDE_calc() {
    ...
    cout << endl;
    cout << "Date file analyzed: " << Form("%s", datafile.Data()) << endl;
